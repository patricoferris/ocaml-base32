let encoded = [ ("helloworld", "NBSWY3DPO5XXE3DE") ]
...
let encoding_test =
  List.map
    (fun (i, o) -> Alcotest.test_case i `Quick (tests_encode i o))
    encoded
...
let decoding_rfc =
  List.map
    (fun (i, _) -> Alcotest.test_case i `Quick (tests_decode i))
    rfc4648_base32

let encoding_hex =
  List.map
    (fun (i, o) ->
      Alcotest.test_case i `Quick
        (tests_encode ~alphabet:Base32.extended_hex i o))
    rfc4648_base32hex
