    ...
    let rec dec j i =
      if i = n then 0
        ...
        let b4, pad =
          ...
          try (dmap x, pad)
          with Not_found when x = padding && pad = 4 -> (0, 5)
        in
        (* [Not_found] iff [x ∉ alphabet and x <> '='] can leak. *)
        let b3, pad =
          let x = get_uint8_or_padding input (i + 2) in
          try (dmap x, pad)
          with Not_found when x = padding && pad = 5 -> (0, 6)
        in
        (* [Not_found] iff [x ∉ alphabet and x <> '='] can leak. *)
        let b2, pad =
          let x = get_uint8_or_padding input (i + 1) in
          try (dmap x, pad)
          with Not_found when x = padding && pad = 6 -> (0, 7)
        in
        (* [Not_found] iff [x ∉ alphabet and x <> '='] can leak. *)
        let b1, pad =
          let x = get_uint8_or_padding input i in
          try (dmap x, pad)
          with Not_found when x = padding && pad = 7 -> (0, 8)
        in
